	...
	async sendSms(phone: string, message: string): Promise<boolean> {
		try {
			...
			if (error instanceof AxiosError) {
				...
				this.logger.error(`SMS xatolik ${phone}: [${status}]`, data || error.message);

				if (status && status >= 500) {
					throw new HttpException("SMS provider xatoligi, keyinroq urinib ko'ring", 502);
				}

				if (status === 400 || status === 422) {
					throw new HttpException(errorPrompt.smsError, 400);
				}
			}

			this.logger.error(`SMS kutilmagan xatolik ${phone}:`, error);
			throw new HttpException(errorPrompt.smsError, 500);
		}
	}
